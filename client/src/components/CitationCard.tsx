...
import { BookIcon, ExternalLinkIcon, ChevronDownIcon, ChevronUpIcon, ShieldCheckIcon } from './Icons';
          ...
          {sources.map((src, index) => (
            ...
            >
              <div className="flex items-center gap-2 truncate">
                <BookIcon className="w-3.5 h-3.5 text-teal-600 shrink-0" />
                <span className="truncate font-medium">{src.title}</span>
              </div>
              <ExternalLinkIcon className="w-3 h-3 text-slate-400 group-hover:text-teal-600 shrink-0 ml-2" />
            </a>
          ))}
        </div>
      )}
